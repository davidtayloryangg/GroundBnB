...
function SingleListing() {
    ...
    useEffect(() => {
        async function fetchListingData() {
            try {
                let { data } = await axios.get(`http://localhost:4000/listings/${listingIdValue}`);
                setListingData(data);
    ...
    if (listingData) {
        ...
        const disableBookedDates = (date : Dayjs) => {
            ...
            for (let booking of bookingsForListingData) {
                let startTime = dayjs((new Timestamp(booking.startTimestamp.seconds, booking.startTimestamp.nanoseconds)).toDate());
                let endTime = dayjs((new Timestamp(booking.endTimestamp.seconds, booking.endTimestamp.nanoseconds)).toDate());
                    ...
                    <Grid item justifySelf='center' maxWidth='5' marginTop='25px'>
                        <Card sx={{ minWidth: 275, alignItems: 'center', justifyContent: 'center'}}>
                            ...
                            <CardActions>
                                ...
                                <Button aria-label="decrease number of people" onClick={decreasePeople}>
                                    <RemoveIcon />
                                </Button>
                                <Typography sx={{ fontSize: 14 }}>
                                    {numberOfPeople}
                                </Typography>
                                <Button aria-label="increase number of people" onClick={increasePeople}>
                                    <AddIcon />
                                </Button>
                            </CardActions>
                            <CardContent>
                                                ...
                                                if (newValue[0] !== null) {
                                                    tempBookings.forEach((booking) => {
                                                        if (newValue[1].isSame(booking, 'date')) {
                                                            conflict = true;
                                                        } else if (booking.isBetween(newValue[0], newValue[1], 'day')) {
                                                            conflict = true;
                                                        }
                                                    });
